...
def show_overfitting_demo():
    ...
    **欠拟合(Underfitting)**是指模型既不能很好地拟合训练数据，也不能很好地泛化到新数据的情况。欠拟合通常是由于模型过于简单，无法捕捉数据中的复杂模式所致。
    ...
    # 显示过拟合vs欠拟合示意图 - 使用图片方式
    overfitting_img_path = os.path.join("img", "overfitting_comparison.svg")
    
    # 检查图片是否存在，不存在则创建
    if not os.path.exists(overfitting_img_path):
        # 确保img目录存在
        os.makedirs(os.path.dirname(overfitting_img_path), exist_ok=True)
