...
def show_bias_variance_demo():
    ...
    # 显示偏差-方差权衡图 - 使用图片方式
    bias_variance_img_path = os.path.join("img", "bias_variance_tradeoff.svg")
    
    # 检查图片是否存在，不存在则创建
    if not os.path.exists(bias_variance_img_path):
        # 确保img目录存在
        os.makedirs(os.path.dirname(bias_variance_img_path), exist_ok=True)
