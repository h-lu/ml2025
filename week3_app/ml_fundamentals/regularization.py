...
def show_regularization_demo():
    ...
    **正则化**是一种防止过拟合的技术，通过向损失函数添加惩罚项来限制模型参数的大小，从而降低模型复杂度。
    ...
    with col2:
        ...
        # 创建文件名，包含参数信息以便缓存不同参数的图像
        regularization_img_path = os.path.join("img", f"regularization_{reg_type_param}_{reg_strength:.2f}.svg")
        
        # 确保img目录存在
        os.makedirs(os.path.dirname(regularization_img_path), exist_ok=True)
